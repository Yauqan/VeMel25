...
#include "better-dynamic-topography.h"
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    BetterDynamicTopography<dim>::execute (TableHandler &)
    {
    ...
    template <int dim>
    std::list<std::string>
    BetterDynamicTopography<dim>::required_other_postprocessors() const
    {
    ...
    declare_parameters (ParameterHandler &prm)
    {
      ...
      {
        prm.enter_subsection("Better dynamic topography");
        {
    ...
    template <int dim>
    void
    BetterDynamicTopography<dim>::parse_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim> void BetterDynamicTopography<dim>::set_last_output_time ( const double current_time )
    {
    ...
    template <int dim> template <class Archive> void BetterDynamicTopography<dim>::serialize ( Archive & ar, const unsigned int )
    {
      ar & last_output_time
    ...
    template <int dim> void BetterDynamicTopography<dim>::save ( std::map<std::string, std::string> & status_strings ) const
    {
      ...
      status_strings["BetterDynamicTopography"] = os.str();
    }

    template <int dim> void BetterDynamicTopography<dim>::load ( const std::map<std::string, std::string> & status_strings )
    {
      if (status_strings.find("BetterDynamicTopography") != status_strings.end())
      {
        std::istringstream is (status_strings.find("BetterDynamicTopography")->second);
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(BetterDynamicTopography,
                                  "better dynamic topography",
